...
multi_seqdoc.pl - Perl cgi script to align and subtractively compare multiple sequence
chromatograms in ABI (Applied Biosystems) format to a single reference chromatogram.

=head1 SYNOPSIS

seqdoc.pl?ref_seq=<reference sequence file>&test_seq0=<test_sequence file>&test_seq1=<test_sequence file> etc.

Designed to be called as a cgi script from a web form with the filenames
...
Aligns each test trace to the reference trace
...
Outputs aligned images of the input sequences and the difference profiles
in html format
...
Written by Mark Crowe - mcrowe@bioinformatics.org
...
my @test_seqs;
for (0..9) {
	$test_seqs[$_] = $cgi->param('test_seq'.$_) if $cgi->param('test_seq'.$_);
}

# Create temporary files to store the chromatograms
# Reference sequence first
open REF, ">".$temp_dir.$$."ref_seq";
my $ref_fh = $cgi->upload('ref_seq');
print REF while <$ref_fh>;
# Need to close file, otherwise ABI.pm can't access them properly
close REF;

# And test sequences
my $testcount = 0; # Incrementing integer to distinguish test files
for my $testfile (@test_seqs) {
	open TEST, ">".$temp_dir.$$.'test_seq_'.$testcount;
	my $test_fh = $cgi->upload('test_seq'.$testcount);
	print TEST while <$test_fh>;
	# Need to close files, otherwise ABI.pm can't access them properly
	close TEST;
	$testcount++;
}
# $testcount is a count of the number of test sequences. Since it's incremented after 
# the last sequence, need to take it back down by one.
$testcount--;
...
# Extract the data from the reference chromatogram file. Get it as a hash reference
...
my @test_data;
my @differences;
# Set initial alignment length to arbitrarily large value, so all true values are less - final
# used value is smallest, so need to start with largest
my $align_length = 1000000000;
for (0..$testcount) {
	my $test_data = read_chromatogram($temp_dir.$$.'test_seq_'.$_);
	...
	my ($al_length, $diffs) = differences($ref_data, $test_data);
	# Only produce output up to length of shortest sequence
	$align_length = $al_length if $al_length < $align_length;
...
sub align {
	...
	for my $index (0..($trace_length - 1)) {
		...
		# 3 doesn't seem to cause any new problems, and seems to fix compression
		next if $index%3;
		my $offset;
		my $start_pos = $index;
		my $end_pos = $index + 30;
...
sub demac {
	...
	for my $file ($temp_dir.$$."ref_seq", glob($temp_dir.$$."test_seq_*") ) {
		open IN, $file;
		open OUT, ">".$file."_new";
		while (<IN>) {
			if ($. == 1) {
